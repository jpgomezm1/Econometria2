...
El coeficiente de $x_{t-1}$ es el parámetro de ajuste, y estimaciones de $\beta^0$ se pueden obtener de los estimadores de MCO de $\beta$ y $\lambda$. Cabe resaltar que el valor de $\lambda$ indica la velocidad de ajuste, mientras mas cercano sea a 1 la velocidad de ajuste es mas lenta
...
Por lo tanto, el modelo de ajuste parcial implica una forma particular de de rezago distribuido.
...
En caso de que el valor de $z_{t+1}$ no sea conocido, una opción es estimar modelo de predicción para esta variable. Este enfoque causa que el grado de complejidad del problema a resolver aumente, ya que necesitamos encontrar dos modelos.
...
Si queremos hacer el predictor dos pasos adelantes, entonces necesitamos estimar la siguiente ecuación primero,
